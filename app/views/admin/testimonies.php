...
if (isset($_POST['UnAprroveTestimony'])) {
 $id  = $_POST['test_id'];
 $sql = DB::table('testimony')
        ->where('id', $id)
        ->update(array('status' => 0));
 if ($sql) {
 	 echo'<div class="alert alert-success" role="alert">
      You have successfully Un-approved user testimony and now apear live on website to other members and visitors.
      </div>';
 }
}
?>
